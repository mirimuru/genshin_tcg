...
class Game:
    ...
    def _execute_switch(self, action: Action) -> None:
        ...
        if action.target not in player.alive_character_indices():
            raise ValueError("交代先が不正です")
        if action.target == player.active_character_index:
            raise ValueError("現在のキャラクターには交代できません")
        event = CharacterSwitchEvent(action.player_id, player.active_character_index, action.target)
        self._emit_event(event)
        player.active_character_index = action.target
    ...
    def _execute_tuning(self, action: Action) -> None:
        ...
        if target is None or target is DiceType.OMNI:
            raise ValueError("調和先のダイスが不正です")
        if player.dice.count(target) > 0:
            raise ValueError("同じ種類のダイスへは調和できません")
        candidates = [dice_type for dice_type in DicePool.ROLLABLE_DICE_TYPES if dice_type is not DiceType.OMNI and player.dice.count(dice_type) > 0]
        if not candidates:
            raise ValueError("調和元のダイスがありません")
        source = candidates[0]
        player.dice.remove(source, 1)
        player.dice.add(target, 1)

    def _execute_reroll(self, action: Action) -> None:
        player = self.state.players[action.player_id]
        if player.has_rerolled:
            raise ValueError("リロールは1回だけです")
        player.dice = self._roll_dice()
        player.has_rerolled = True
        if all(other.has_rerolled for other in self.state.players):
    ...
    def _end_round(self, player_id: int) -> None:
        self.state.players[player_id].ended_round = True
        if all(player.ended_round for player in self.state.players):
            event = RoundEndEvent()
            self._emit_event(event)
            for player in self.state.players:
    ...
    def _start_roll_phase(self) -> None:
        ...
        self.state.first_player = self.rng.choice([0, 1])
        self.state.current_player = self.state.first_player
        for player in self.state.players:
            player.dice = self._roll_dice()
            player.has_rerolled = False

    def _roll_dice(self) -> DicePool:
        return DicePool({self.rng.choice(DicePool.ROLLABLE_DICE_TYPES): 1 for _ in range(8)})
