...
class SummonDefinition(ABC):
    ...
    召喚物のルールは定義クラスに、現在の使用回数などの可変値は
    ``SummonInstance`` に保持する。
    """
...
class SummonInstance:
    ...
    def __init__(
        self,
        definition: type[SummonDefinition] | SummonDefinition,
        *,
        ...
        if not isinstance(self.definition, SummonDefinition):
            raise TypeError(
                "召喚物定義はSummonDefinitionまたはそのサブクラスである必要があります"
            )
...
class SummonRegistry:
    ...
    def __init__(
        self,
        definitions: Sequence[type[SummonDefinition] | SummonDefinition] = (),
    ):
