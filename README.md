...
`ChanceOutcome` は確率的に発生する結果と確率を保持し、`expected_value()` は確率の合計を検証した上で期待値を計算します。現段階では汎用的な確率探索プリミティブとして用意しており、実際のダイスロール等への接続は次工程です。
...
確率結果の共通基盤として `ChanceOutcome` / `expected_value()` を追加しました。今後、ダイス・リロール・ランダムカード等の実際のゲーム結果をこの基盤へ接続し、expectimax系探索へ拡張します。
...
状態コピー・シミュレーション、確率結果、カードTarget、装備、元素反応、イベント、合法手生成、状態評価、CPU探索などを含むテストを実施しています。

## 今後の予定

1. 確率結果をダイス・リロール等のゲームシステムへ接続
2. 確率結果を含むexpectimax系探索を実装する
3. 探索性能を改善し、深い探索を安定して実行できるようにする
