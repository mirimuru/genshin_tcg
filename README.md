...
確率結果の共通基盤として `ChanceOutcome` / `expected_value()` を追加し、さらに実際のダイスロールを `simulate_roll()` から確率的Game状態へ展開できるようにしました。次工程では、このChance NodeをCPU探索そのものへ統合してexpectimax系探索へ拡張します。
...
状態コピー・シミュレーション、確率結果、ダイスChance Node、カードTarget、装備、元素反応、イベント、合法手生成、状態評価、CPU探索などを含むテストを実施しています。

## 今後の予定

1. 確率的Game状態をCPU探索のChance Nodeへ統合する
